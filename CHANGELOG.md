...
- Round 440 — **External-data interplay + robustness suite**: 5 more integration tests pinning the corners where external media meets the rest of the writer/reader — a `cmov` compressed movie resource carrying an external track (the dref table + external chunk offsets survive the QTFF pp. 80–81 round-trip; external offsets are layout-independent so the mdat-first ordering constraint is trivially satisfied), `stz2` compact sample sizes sourced from the external locations (4-bit form, `sample_size_source` discriminator confirmed), a byte-patched **dangling** `data_reference_index` pinning the long-standing lenient-local contract (classifies non-external, resolution never engages, the attached opener is provably never consulted), a truncated external file (early samples read, later ones yield recoverable per-sample truncation errors — never a crash or silent short read), and a self-contained vs external authoring **equivalence** check (identical media presented both ways must produce identical packet streams — data, pts, dts, duration, keyframe — through the `Demuxer` trait)
