...
- opt-in mvex/mehd total fragmented duration (§8.8.2)
- opt-in per-traf tfdt decode-time baselines (§8.8.12)
- opt-in mfra/tfra/mfro fragment random-access index (§8.8.9–§8.8.11)
- fragmented trun composition-time offsets (§8.8.8)
- resolve §8.8.7 base-data-offset against the data reference
- reference-movie chain composes with external-data resolution
- document external-data movies (authoring + resolution)
- external-data interplay + robustness suite
- hermetic external data-reference resolution pass
- external data-reference resolution (set_data_reference_opener)
- external-media authoring (set_external_media)
- structure-aware mux target — round-trip contract under fire
- muxer layout docs reflect the configurable placement + chunking
- pin round-trip fixtures — multitrack interleaved+faststart, edit-listed, B-frame ctts
- ffprobe black-box parity for interleaved + faststart layouts
- Muxer-trait integration — mov joins the registry write path
- moov-before-mdat faststart placement (MoovPlacement, QTFF p. 365)
- time-interleaved mdat chunking (ChunkStrategy, QTFF p. 358)
- generalize the sample-data layout to a planned multi-chunk mdat
...
- Round 443 — **`mehd` write path (ISO/IEC 14496-12 §8.8.2)**: opt-in `MovMuxer::with_fragment_total_duration()` opens the fragmented init segment's `mvex` with a Movie Extends Header Box declaring the total fragmented-movie duration — §8.8.2.3's "duration of the longest track, in the timescale indicated in the Movie Header Box", computed exactly (per-track media-tick sums rescaled round-half-up into `mvhd.time_scale`, maximum wins) since every sample is supplied before encoding; v0→v1 auto-promotion past 32 bits, written before the `trex` records per the §8.8.1 order. Round-trips onto `MovDemuxer::mehd`. Off by default (the omitted-box shape is the live/unknown-duration case and preserves historical output). 1 new test
- Round 443 — **`tfdt` write path (ISO/IEC 14496-12 §8.8.12)**: opt-in `MovMuxer::with_fragment_decode_times()` emits a Track Fragment Decode Time Box in every `traf` (between `tfhd` and the first `trun` per the §8.8.6.2 order) carrying the absolute `baseMediaDecodeTime` of the fragment's first sample — "the sum of the decode durations of all earlier samples in the media" — with v0→v1 auto-promotion past 32 bits; honoured on local and external trafs alike, and the baseline's magnitude is known before the two-pass moof sizing so the width choice never desynchronizes the offset math. The demuxer's `moof` walk already prefers a present `tfdt` over its running cursor, so values round-trip; the new mid-stream-join test pins the §8.8.12 point: init segment + only the second media segment keeps dts 200/300 with `tfdt`, degrades to a re-zeroed 0/100 without it. Off by default (optional box; historical output shape preserved byte-for-byte). 2 new tests
- Round 443 — **`mfra`/`tfra`/`mfro` write path (ISO/IEC 14496-12 §8.8.9–§8.8.11)**: opt-in `MovMuxer::with_fragment_index()` appends the trailing Movie Fragment Random Access Box to `encode_fragmented_to_vec` output — one `tfra` per track indexing the **first sync sample** of each of that track's fragments (`time` = presentation time `dts + composition_offset` in media-timescale ticks per §8.8.10.3, `moof_offset` = the "first byte of the enclosing Movie Fragment Box", and the 1-based `traf_number`/`trun_number`/`sample_number` structural coordinates, correct across the multi-`trun` external shape; fragments with no sync sample for a track contribute no entry), each `tfra` auto-promoting v0→v1 when a time or offset leaves 32 bits, structural fields written 4 bytes wide, entries naturally in increasing-time order (the per-track DTS cursor climbs across fragments), closed by the mandatory `mfro` whose `size` is the whole `mfra` length so readers locate the box from the end of the file. The demuxer's `tfra`-driven fragmented seek (binary search) now engages on our own output — external-data tracks included, since a `tfra` locates samples structurally rather than by data offset. Off by default: the box is optional and the historical output shape is preserved byte-for-byte. 3 new tests (opt-in absence, wire shape + parsed-entry/moof-position/mfro-size cross-check + tfra-driven seek composing with external resolution, no-sync-fragment omission)
- Round 443 — **Fragmented trun composition-time offsets (ISO/IEC 14496-12 §8.8.8)**: the fragmented write path no longer drops `MuxSample.composition_offset` — each `trun` now applies the non-fragmented `ctts` policy per run (§8.8.8.1 points the signed-offset recommendations back at the composition time-to-sample box): the `sample_composition_time_offset` field is omitted entirely when every offset in the run is zero (the historical 12-byte-row wire shape is preserved byte-for-byte), an all-non-negative run emits version 0 (unsigned), and any negative offset auto-promotes that run's `trun` to version 1 (signed `int(32)`). B-frame reorder therefore round-trips PTS exactly on the fragmented path too — including on external-data tracks, whose `MuxSample`s carry the composition-offset axis while their byte axis lives in the external file. 4 new tests (v0 round-trip incl. demuxer-side dts/cts, per-run v1 promotion, all-zero wire stability, external fragmented reorder)
- Round 443 — **Fragmented external-data semantics (ISO/IEC 14496-12 §8.8.7)**, the r440 follow-up: the base-data-offset now resolves against each track fragment's **effective data reference** (the `dref` entry named by the tfhd/trex sample-description index), closing the combination r440 had rejected pending the spec text. Read side: an explicit `tfhd` `base_data_offset` "is a data offset that is identical to a chunk offset in the Chunk Offset Box" (§8.8.7.1) and a chunk offset addresses "its containing media file" (§8.7.5.3) — so with a non-self `dref` entry the resolved per-run offsets land in the **external** file, sanctioned by §8.8.4.1 ("possible to build incremental presentations where the media data is in files other than the file containing the Movie Box"); the resolved fragment samples then flow through the r440 `set_data_reference_opener` machinery unchanged (recoverable per-sample `Unsupported` without an opener, sticky caching with one). The moof-relative modes are anchored at byte positions of the fragment's own file, so `default-base-is-moof` with a non-self effective entry is refused at open, an inheriting traf ("the end of the data defined by the preceding track fragment") must use the **same data reference** as the traf it inherits from — §8.8.7.1: "must all use the same data-reference (i.e., the data for these tracks must be in the same file)" — with the first inheriting traf (anchored at the moof's own first byte) required local, while two trafs naming the same external file chain legally within it; `duration-is-empty` trafs define no data and are exempt. New typed `Tfhd::addressing()` → `TrafAddressing { ExplicitBase, MoofRelative, Inherited }`. Write side: `encode_fragmented_to_vec` now honours `set_external_media` — an external track's traf carries an explicit `base_data_offset` anchored at the fragment's lowest location offset plus one `trun` per byte-contiguous location run (sizes from the locations, timing/keyframe from the samples; a fragment whose locations spread past `i32::MAX` bytes is rejected per the signed §8.8.8.2 `data_offset`), local tracks keep `default-base-is-moof`, and no external bytes land in any fragment's `mdat`. 11 new integration tests: wire-shape (explicit-base vs moof-relative trafs, multi-trun gap splitting), opener round-trip + per-sample recoverability + fragmented seek, self-contained vs external fragmented authoring equivalence, the three §8.8.7.1 refusal shapes (byte-patched `data_reference_index` / tfhd flags), a legal all-local inheritance chain, and a hand-built moof chaining an inherited traf after an explicit-base traf inside the external file across data-offset-less runs

- Round 440 — **External-data interplay + robustness suite**: 5 more integration tests pinning the corners where external media meets the rest of the writer/reader — a `cmov` compressed movie resource carrying an external track (the dref table + external chunk offsets survive the QTFF pp. 80–81 round-trip; external offsets are layout-independent so the mdat-first ordering constraint is trivially satisfied), `stz2` compact sample sizes sourced from the external locations (4-bit form, `sample_size_source` discriminator confirmed), a byte-patched **dangling** `data_reference_index` pinning the long-standing lenient-local contract (classifies non-external, resolution never engages, the attached opener is provably never consulted), a truncated external file (early samples read, later ones yield recoverable per-sample truncation errors — never a crash or silent short read), a self-contained vs external authoring **equivalence** check (identical media presented both ways must produce identical packet streams — data, pts, dts, duration, keyframe — through the `Demuxer` trait), and a **two-layer indirection composition**: a thin `rmra` reference movie whose `rdrf` url resolves through `open_with_aliases` to an external-data movie whose per-track `dref` then resolves through `set_data_reference_opener`

- Round 440 — **Fuzz: hermetic external-resolution pass**: the `demux` cargo-fuzz target grows a fourth drain that touches `track_has_external_data` per track, attaches a data-reference opener that unconditionally fails **without touching the filesystem or network** (the harness stays hermetic — fuzz input controls the dref url bodies), and walks the bounded packet queue continuing over the recoverable per-sample errors, so the per-`(track, dref-entry)` resolution + sticky-failure cache faces hostile dref tables (dangling `data_reference_index`, oversized entry counts, non-UTF-8 url bodies) on every input

- Round 440 — **Demuxer external-data resolution** (`MovDemuxer::set_data_reference_opener`), closing the read side of external-data movies: a sample whose description points at a non-self `dref` entry (QTFF p. 94 — self-reference flag clear means the media is *not* "in the same file as the movie atom") has long surfaced a recoverable `Unsupported` error; an attached opener now resolves it — the demuxer hands the opener the parsed typed `DataReference` (`url ` / `urn ` / `alis` / `rsrc`), gets back a `Box<dyn ReadSeek>` over the external file, and reads each sample's bytes at its chunk-derived offset within *that* file (QTFF p. 79 — each sample's "byte offset from the data reference"; p. 113 — chunk offsets index the data stream the reference designates, "This permits referring to media data in files without any atom structure"). Security model matches `open_with_aliases`: the default `open` path can never reach the filesystem or network — resolution is an explicit opt-in, and URL→stream policy is entirely the opener's (QTFF's Reference Movies chapter says a `url ` "can be absolute or relative, and can specify any protocol that QuickTime supports" but leaves resolution to the player). The opener runs **at most once** per distinct `(track, dref-entry)` pair: successes cache the reader, failures cache the error text and replay it as a recoverable per-sample `Unsupported` (a hostile movie with thousands of external samples cannot hammer the opener) — local tracks keep demuxing either way, and re-attaching clears the cache. New built-in `dref_file_opener(base_dir)` implements a deliberately narrow sandboxed local-file policy: `file://` URLs through the same rules as `open_file_url`, relative URLs percent-decoded then joined to the caller-designated base directory only when every component is a plain name (no absolute paths, no `..` traversal, no drive/rooted shapes — a hostile movie cannot escape the sandbox), everything else (`http://` / `rtsp://` / Macintosh `alis`/`rsrc` records, whose binary layout is outside the QTFF spec) rejected with `ErrorKind::Unsupported`. Seek, the applied-edit-list mode, every `ChunkStrategy` × `MoovPlacement` combination, and the `Demuxer`-trait packet path all compose with resolution (the sample queue is source-agnostic; only the byte read routes to the external reader). New `has_data_reference_opener` accessor; `DataReferenceOpener` type alias exported. 10 integration tests (`tests/synth_round440_external_media.rs`): mixed local+external movie round-trip with opener-call counting, no-opener recoverable errors, sticky failure caching, chunk coalescing geometry + per-sample offsets, `co64` promotion for >4 GiB external offsets, on-disk relative-sidecar resolution through `dref_file_opener`, sandbox rejection matrix (traversal / absolute / foreign-scheme / alias / locationless urn / no-base-dir), seek-then-drain on a resolved external track, faststart+interleaved coexistence, and edit-list-applied timing over external bytes

- Round 440 — **Muxer external-media authoring** (`MovMuxer::set_external_media`), the write side of QTFF's appendix "Defining Media Data Layouts": *"A QuickTime file can reference media data stored in a number of files, including the file itself"*, addressed *"by file offset, rather than by a data structuring mechanism of a particular file format"* — so the external file may be any format (a raw elementary stream, another movie, …). A track declared external writes **none** of its sample bytes into this file's `mdat`; the chunk-offset table carries the caller-supplied `ExternalSampleLocation { offset, size }` rows (offsets into the external file, per QTFF p. 79 — each sample's "byte offset from the data reference" — and p. 113 — chunk offsets are into the data stream the data reference designates), byte-contiguous locations coalesce into multi-sample chunks for a compact run-length `stsc`, offsets past the 32-bit range auto-promote the movie to `co64`, and every sample entry's `data_reference_index` points at the designated non-self `dref` entry (the `url `/`urn ` naming the external storage) instead of a self-reference. Samples are added with empty `data` (the `MuxSample` keeps supplying duration / keyframe / composition-offset; the location supplies offset + size, which also feed `stsz`/`stz2`). `set_data_references` accordingly relaxes from "exactly one SelfRef" to "**at most one** SelfRef": a table with none is now legal at set time but must be paired with `set_external_media` by encode time (new cross-track validation on both encode entry points, so the two calls compose in either order; re-setting the table re-validates a standing external marking). Guard rails: the external `data_reference_index` must name an in-range non-self entry, location count must equal sample count, in-file sample bytes are rejected, sample-aux streams conflict in both call orders (the aux slab lives in this file's `mdat`, which an external track does not touch), and the fragmented write path rejects external tracks (fragment data addressing is relative to the fragment's own file). Works under every `ChunkStrategy` × `MoovPlacement` combination (external tracks are excluded from interleaving — they have no bytes in this file to interleave — and their fixed external offsets are independent of the faststart `moov` sizing loop). 8 unit tests + a structure test pinning empty-`mdat` + verbatim external `stco` + `data_reference_index` targeting; the read side surfaces such samples as recoverable `Unsupported` errors exactly as before

- Round 429 — **Write-side fuzz target (`mux`)**: a second cargo-fuzz harness alongside `demux`, structure-aware — the fuzz bytes decode into a bounded valid-by-construction mux recipe (1–3 tracks × 1–48 samples, arbitrary 16-bit movie/media timescales, signed composition offsets, every `ChunkStrategy` × `MoovPlacement` combination) and the contract is strict round-trip: encode must succeed, the output must demux, per-track payload streams must be byte-identical in order, every resolved sample offset must land in-bounds with the exact sample bytes at it, and `is_faststart()` must match the requested placement. Puts the round-429 arithmetic under fire: the interleave period rescale across hostile timescale ratios, saturating decode-time accumulation, the k-way chunk merge, run-length `stsc` / multi-entry `stco` emission, and the faststart fixed-point sizing loop. 150-second local run after landing: 837 K executions clean. The daily fuzz workflow now splits its budget across both targets

- Round 429 — **Pinned round-trip fixtures** (`tests/fixtures/rt_r429_*.mov`): three byte-pinned muxer-output movies — video+audio **multi-track interleaved+faststart**, **edit-listed** (presentation delay + head trim + dwell), and **B-frame `ctts`** (negative offsets ⇒ v1 box, plus auto-`cslg`). The muxer writes no wall-clock timestamps, so each fixture is a pure function of its builder inputs: the committed bytes must equal the in-test deterministic rebuild (any unintended write-path change is a byte diff; intended changes regenerate via `OXIDEAV_MOV_REGEN_FIXTURES=1`). Each fixture also re-demuxes with its invariants asserted (faststart flag + chunk geometry + packet survival; typed edit-list summaries; exact per-sample dts/pts recovery through the v1 `ctts`), and all three must pass silent black-box `ffprobe` probing (skipped when absent). 4 tests

- Round 429 — **ffprobe black-box acceptance for the new layouts**: interleaved, faststart, and faststart+interleaved two-track (raw-video + twos-audio) movies are handed to `ffprobe` as an opaque validator; each file must be accepted with a silent `-v error` run and its complete packet table — per-stream `(pos, size)` row sequences and the whole-file row multiset — must equal our own demuxer's view byte-for-byte. Empirical oracle notes pinned in-test: the oracle emits `packet=` fields in its own section order (`stream_index,size,pos`) regardless of the requested order, and schedules cross-stream packet emission by timestamp where we walk file offsets — so parity is per-stream sequence + multiset, both exact. A fourth test pins that both `moov` placements present identical content to the oracle. 4 oracle tests, skip silently without `ffprobe` on `$PATH`

- Round 429 — **Registry `Muxer` integration (dual-API write side)**: `register_containers` / `register(&mut RuntimeContext)` now install a `mov` **muxer** factory next to the long-standing demuxer, so `ContainerRegistry::open_muxer("mov", output, &streams)` works fleet-uniformly — while the direct `MovMuxer` API remains the equally-supported second half of the dual-API convention. The new `registry::MovRegistryMuxer` adapter buffers packets (the non-fragmented QTFF layout needs complete sample tables before finalization; `write_header` validates, `write_packet` accumulates, `write_trailer` encodes and writes once): per-stream media timescale from the time base denominator (numerator folded into tick scaling), `ctts` composition offsets recovered from pts−dts, missing packet durations recovered from decode-time deltas (final sample reuses the previous duration), codec-header-flagged packets skipped, and `stsd` codec-config extension atoms carried verbatim from `extradata` (the demuxer's symmetric surface). Codec identity → QTFF sample-description FourCC via a case-correct codec-id table (QTFF sound formats `twos`/`sowt`/`fl32`/`fl64`/`in24`/`in32`/`ulaw`/`alaw`/`lpcm`-family plus the conventional ISO video entry types), falling back to the stream's `CodecTag::Fourcc` folded back to lowercase (the tag canonicalises to uppercase; QTFF spellings are conventionally lowercase); unmappable streams and non-A/V media types are rejected at open. 5 tests: full demux → registry-mux → demux round-trip (bytes, pts, dts/`ctts`, on-wire formats), state-machine enforcement, unmappable-codec rejection, duration recovery, registry-installation pin

- Round 429 — **Moov-before-mdat ("faststart") placement** (QTFF "Optimizing … for Web Playback", p. 365: "The important change … was for QuickTime to place global movie information at the beginning of the file"): new `MoovPlacement` on the non-fragmented write path — `AfterMdat` (default, historical `ftyp`+`mdat`+`moov`) or `BeforeMdat` (`ftyp`+`moov`+`mdat`), via `with_moov_placement` / the `with_faststart()` convenience / the `moov_placement()` getter. Chunk offsets are file-absolute, so moov-first makes them depend on the `moov`'s own size; the muxer resolves that with a fixed-point sizing pass (the `moov` length depends only on offset-field *widths* — `stco` vs `co64`, `saio` v0 vs v1 — never on the offset values, so the loop is monotone and converges in a couple of iterations). Combines freely with `ChunkStrategy` interleaving, edit lists, and sample-aux streams; rejected in combination with the compressed movie resource (`cmov`) since a zlib-compressed `moov`'s size depends on the very offsets it stores. The demuxer's existing `is_faststart()` classifies the result. 9 round-trip tests (top-level atom order, faststart classification, placement-invariant packets, byte-level offset resolution, interleave/edit-list/aux combinations, cmov rejection)

- Round 429 — **Time-interleaved `mdat` chunking** (QTFF "Interleaving Movie Data", p. 358): the non-fragmented write path previously always laid one chunk per track, tracks back-to-back — fine in memory, but the spec is explicit that "in order to get optimal movie playback, you must create the movie with interleaved data" so that "the data for a particular time in the movie is close together in the file". New `ChunkStrategy` (builder `with_chunk_strategy`, getter `chunk_strategy`): `SingleChunkPerTrack` (default, historical layout, byte-identical output) or `InterleaveByMovieTicks(period)` — each track is cut into chunks of at most `period` movie-timescale ticks of decode time (converted round-half-up into the track's media timescale, floored at one sample) and all tracks' chunks merge into `mdat` by ascending start time, ties in `add_track` order; the spec's worked example (half-second audio runs) is `InterleaveByMovieTicks(300)` at the default 600 movie timescale. Underneath, the whole layout pass is rebuilt on a planned-segment model: `stsc` now run-length compresses consecutive equal samples-per-chunk runs (§8.7.4 / QTFF p. 76) and `stco`/`co64` emit one entry per chunk (§8.7.5 / QTFF p. 78) instead of hardcoding the single-entry forms. Sample-aux slabs stay contiguous per track (interleaved files place them after the last chunk, in track order) so the single-entry `saio` contract (§8.7.9.3) is preserved. 2 unit + 8 round-trip tests (chunk counts/geometry, file-order alternation, per-chunk packet emission switches, layout-invariant per-track bytes and timelines, zero-period rejection, one-sample floor, aux-slab contiguity under interleave)
...
- Round 417 — **Fuzz harness extended to the never-presented (discard) surface**: after the applied edit-list pass, the `demux` target now enables `emit_never_presented(true)` and runs a third bounded drain + seek — the extrapolation mapper (`never_presented_timing_for_sample`) joins the `next_packet` hot path under that switch and must survive attacker-controlled edit lists alongside the presented-path mapper; the typed elst surface (`Track::elst_version`, `edit_start_delay` / `edit_media_start` / `edit_total_duration` saturating sums) is touched per track. The fuzz crate's lockfile also moves `oxideav-core` 0.1.26 → 0.1.30 (the discard-flag packet builder landed after .26). 150-second local run after the extension: 764 K executions clean

- Round 417 — **`mvhd` QuickTime headroom: movie display matrix + preview/poster/selection/current fields** (QTFF pp. 33–34): `parse_mvhd` previously skipped the 36-byte matrix and read the six QuickTime time fields as opaque `pre_defined` padding; both now surface typed on `Mvhd` — `matrix: [i32; 9]` (same `[a b u; c d v; tx ty w]` 16.16/2.30 convention as `Tkhd::matrix`, with a new `Mvhd::rotation()` reusing the shared coarse-rotation classifier) plus `preview_time` / `preview_duration` / `poster_time` / `selection_time` / `selection_duration` / `current_time` (movie-timescale ticks; zero on ISO BMFF files where §8.2.2 calls the bytes `pre_defined`). Write symmetry on `MovMuxer`: `set_movie_matrix(Option<[i32;9]>)` (None = identity default) and `set_movie_preview` / `set_movie_poster_time` / `set_movie_selection` / `set_movie_current_time`, honoured by both the non-fragmented `moov` and the fragmented init `moov`. Truncated-matrix payloads now error instead of under-running the skip. 3 unit + 3 round-trip tests

- Round 417 — **ffprobe black-box parity suite for edit-list timelines** (priming/delay, head-trim, delay+trim, and dwell fixtures): muxed `raw `-video movies with real edit lists are handed to `ffprobe` as an opaque validator and its packet list compared against the applied edit-list mode. Empirical oracle contract (probed, not assumed): the installed build emits only the *presented* samples of all-keyframe media — so the suite asserts our presented (non-discard) pts sequence equals the oracle's complete list one-to-one (delay offsets included) AND that the round-417 discard emission accounts for exactly the samples the oracle dropped (nothing silently lost on either side). The dwell fixture is asserted at the level the oracle supports (container accepted, all samples survive; the installed build plays a dwell on the raw media timeline) with our own §8.6.6.3 dwell contract pinned in-tree. 4 oracle tests, skip silently without `ffprobe` on `$PATH`

- Round 417 — **Muxer `edts/elst` full-semantics parity** (QTFF pp. 46–48): typed constructors complete the write surface — `MuxEdit::dwell(track_duration, media_time)` (the `media_rate == 0` single-frame hold, ISO/IEC 14496-12 §8.6.6.3) and `MuxEdit::segment_with_rate(track_duration, media_time, media_rate)` (arbitrary positive 16.16 rate per the QTFF Chapter 5 pp. 226–227 consumption model) — both round-tripping through the demuxer's segment classification and applied-mode timing. `set_edit_list` now rejects **negative** `media_rate` (QTFF p. 48 forbids it and the read path drops such segments per-segment; 0 stays legal as the dwell signal). And the **fragmented write path emits the `edts > elst` into the init-segment `trak`** (between `tkhd` and `mdia`, same auto-versioned builder as the non-fragmented path) — previously edit lists were silently ignored on `encode_fragmented_to_vec`, so a fragmented presentation lost its priming-skip / start-delay edits; the `moof`s only index samples, so the init `moov` is where the list must ride. 6 round-trip tests (dwell classification, rate-2.0 applied timing, negative-rate rejection, fragmented edts + typed summaries, fragmented v1 auto-promotion, no-edts-when-empty pin)

- Round 417 — **Discard-flagged never-presented media in the applied edit-list mode** (QTFF pp. 46–48): the edit list describes what is *presented*, but media outside every segment is often still *required for decoding* — the sync sample a head-trimmed segment's first presented frame depends on, or the encoder-priming audio an empty edit skips — and the applied mode alone dropped it, starving a real decoder. New opt-in `MovDemuxer::emit_never_presented(true)` (query `never_presented_emitted()`): `next_packet()` then emits those samples with the packet **discard flag** set (`Packet::is_discard`) and timing extrapolated from the *nearest presenting Media segment* — head-trimmed media lands at negative edited pts (decodes before the presentation starts), tail-trimmed media past the presentation end, all at the segment's `media_rate` scaling, dts keeping the rate-scaled composition offset. The per-sample core is the new `edit::never_presented_timing_for_sample` (crate-root export; per-track wrapper `MovDemuxer::never_presented_timing_for`), saturating on hostile 64-bit input; lists with no presenting Media segment (dwell-only / all-empty) still drop, and the switch is inert while the applied mode is off. Edited-timeline `seek_to` honours the mode: a landing sync sample that no edit presents reports its extrapolated discard dts (that sample IS the next packet). 5 unit + 7 muxer→demuxer round-trip tests (head trim / tail trim / delay+trim splice / dwell-only / both mode-off contracts / seek-onto-discarded-sync)

- Round 417 — **Typed `elst` surface** (QTFF p. 47): new `parse_elst_full` preserves the FullBox header — `Elst { version, flags, entries }` — so a caller can tell a *zero-entry* `elst` from a missing `edts` atom and keep the author's v0/v1 width choice when re-muxing (`parse_elst` stays as the entries-only wrapper). The demuxer records the on-wire version on the new `Track::elst_version` (`None` = no `edts/elst`). New list-summary helpers, exported at crate root and wrapped as `Track` accessors: `initial_empty_duration` / `Track::edit_start_delay` (movie-tick presentation delay declared by the leading empty edits — the encoder-priming-skip / delayed-start idiom), `first_presented_media_time` / `Track::edit_media_start` (the head-trim media tick of the first presenting edit), and `total_edit_duration` / `Track::edit_total_duration` (the declared presentation span `tkhd.duration` must equal per QTFF p. 41) — all saturating on hostile 64-bit sums. Plus the named constants `EMPTY_EDIT_MEDIA_TIME` (-1) and `MEDIA_RATE_ONE` (0x0001_0000). 3 unit + 2 demux-level tests
