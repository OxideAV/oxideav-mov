...
///
/// `(item_id, x, y)`: the contributing tile item plus the top-left
/// canvas coordinate where the renderer will blit the tile's pixels.
...
/// item's `ispe`; the spec mandates every tile share the same encoded
/// extent, so we don't try to reconcile mismatched per-tile `ispe`
/// records (the renderer's existing `render_grid` enforces the same
/// rule on the decoded-buffer side).
...
/// §6.6.2.3.3, but we don't validate the rest (the renderer does that
/// once it has the decoded buffers).
///
/// Returns `Err(InvalidData)` when:
///
/// * the item has no `idat`-resident grid payload (we don't follow
///   `construction_method == 0` here — callers wanting that path
///   should resolve the bytes themselves and then call
///   [`parse_grid`] + the planner directly);
...
/// row/column index using these dimensions.
...
/// [`Overlay::offsets`] in the same index order.
///
/// Returns `Err(InvalidData)` when:
///
/// * the item has no `idat`-resident overlay payload;
...
pub fn plan_overlay_layout(meta: &BmffMeta, item_id: u32) -> Result<OverlayLayout> {
    ...
    let dimg_targets = meta.derived_from(item_id);
    let overlay = parse_overlay_with_source_count(&raw, dimg_targets.len())?;
